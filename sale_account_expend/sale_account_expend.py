...
class sales_orders_loan(models.Model):
	...
	def _amount_all(self):
		for order in self:
			...
			if self.is_loan and (self.amount_month2 > 0):
				count = total + self.amount_grant - amount_down_payment
				if self.down_payment > 0:
					count = total + self.amount_grant - amount_down_payment - self.down_payment					
				...
				if (self.amount_repayment_rate1 > 0.0) or (self.amount_repayment_rate2 > 0.0):
					...
					loan_total = self.amount_installment1 * self.amount_month1 + self.amount_installment2 * self.amount_month2
				else:
					rate2 = self.amount_rate2 / 100.0

					#前期等于0.0，月本金就是商品总价/还款期月份数
					if self.amount_rate1 == 0.0:
						rate1 = 0.0
						month_capital = count / self.amount_month2
						self.amount_installment1 = 0.0

					#前期大于0.0，月本金就是商品总价/全部月份数
					else:
						rate1 = self.amount_rate1 / 100.0
						month_capital = count / (self.amount_month1 + self.amount_month2)
						self.amount_installment1 = count * rate1 + month_capital

					self.amount_installment2 = count * rate2 + month_capital
					loan_total = count * rate1 * self.amount_month1 + count * rate2 * self.amount_month2 + count
...
class account_invoice_loan(models.Model):
	...
	@api.constrains('amount_grant')
	def _check_max_amount_grant(self):
		if self.amount_grant > 3000:
			raise UserError(_("生活补助款最高额度为3000"))
